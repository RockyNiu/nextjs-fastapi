...
class TestBaseDAO(DaoTest):
    ...
    def init_dao(self) -> None:
        ...
        self.dao: Optional[BaseDAO] = None  # BaseDAO is abstract, we'll create instances in individual tests
    ...
    def test_init_without_session_uses_database_manager(self, test_session: Session) -> None:  # type: ignore[misc]
        """Test BaseDAO initialization without session uses DatabaseManager."""
        with patch.object(DatabaseManager, 'get_session', return_value=test_session) as mock_get_session:
            dao = BaseDAO()
            
            assert dao.session is test_session
            mock_get_session.assert_called_once()
    
    def test_init_with_none_session_uses_database_manager(self, test_session: Session) -> None:  # type: ignore[misc]
    ...
    def test_database_session_not_initialized_error_handling(self) -> None:  # type: ignore[misc]
        ...
        with patch.object(DatabaseManager, 'get_session', side_effect=DatabaseSessionNotInitializedError()):
...
class TestBaseDaoInheritance(DaoTest):
    ...
    def test_concrete_dao_inherits_session_management(self, test_session: Session) -> None:  # type: ignore[misc]
