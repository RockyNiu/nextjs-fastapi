...
from app.core.security import get_password_hash, generate_password_reset_token, generate_email_verification_token
...
class TestUserDAO(DaoTest):
    ...
    def test_init_without_session(self, test_session: Session) -> None:  # type: ignore[misc]
        ...
        with patch('app.db.database.DatabaseManager.get_session', return_value=test_session):
            dao = UserDAO()
    ...
    def test_get_by_email_existing_user(self, test_session: Session) -> None:  # type: ignore[misc]
        ...
        with patch.object(test_session, 'query') as mock_query:
            mock_query.return_value.filter.return_value.first.return_value = mock_user_orm
    ...
    def test_reset_password_by_token_expired(self, test_session: Session) -> None:  # type: ignore[misc]
        ...
        with patch.object(test_session, 'query') as mock_query:
            mock_query.return_value.filter.return_value.first.return_value = None  # Query would return None for expired token
