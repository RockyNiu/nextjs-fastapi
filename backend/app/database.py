...
class DatabaseSessionNotInitializedError(Exception):
  ...
  """
  def __init__(self):
    super().__init__("Database session not initialized. Use `DatabaseManager.get_session()` to initialize it.")
  ...
  def initialize(cls):
    if not cls._initialized:
      ...
      cls.engine = create_engine(
        f"mysql+mysqlconnector://{config['username']}:{config['password']}@{config['host']}:{config['port']}/{config['database']}?charset=utf8mb4"
      )
